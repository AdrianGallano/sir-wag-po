...
interface CartSheetProps {
  isOpen: boolean;
  onClose: () => void;
  cartStockss: CartStocks[];
  refreshCart: () => void;  // Added this prop to refresh the cart after deletion
}

const CartSheet: React.FC<CartSheetProps> = ({ isOpen, onClose, cartStockss, refreshCart }) => {
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      for (const product of cartStockss) {
        const productOrder = {
  ...
  return (
    <div className={`fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-end ${!isOpen ? 'hidden' : ''}`}>
      <div className="w-1/3 h-full bg-white p-6 overflow-y-auto">
        ...
        {/* Cart Products */}
        <div>
          {cartStockss.length > 0 ? (
            cartStockss.map((product) => (
